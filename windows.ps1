...
$latexmkrcContent = @'
    ...
    ${1}latex = 'uplatex %O -kanji=utf8 -no-guess-input-enc -synctex=1 -interaction=nonstopmode %S';
    #
    ${1}uplatex = 'uplatex %O  -synctex=1 -interaction=nonstopmode %S';
    # pdfLaTeX のビルドコマンド
    ${1}pdflatex = 'pdflatex %O -synctex=1 -interaction=nonstopmode %S';
    # LuaLaTeX のビルドコマンド
    ${1}lualatex = 'lualatex %O -synctex=1 -interaction=nonstopmode %S';
    # XeLaTeX のビルドコマンド
    ${1}xelatex = 'xelatex %O -no-pdf -synctex=1 -shell-escape -interaction=nonstopmode %S';
    # Biber, BibTeX のビルドコマンド
    ${1}biber = 'biber %O --bblencoding=utf8 -u -U --output_safechars %B';
    #${1}bibtex = 'upbibtex %O %B';
    ${1}platex= 'platex %O -synctex=1 -halt-on-error -interaction=nonstopmode -file-line-error %O %S';
    #${1}bibtex = 'upbibtex %O %B';
    ${1}bibtex = 'pbibtex %O %B';
    # makeindex のビルドコマンド
    ${1}makeindex = 'upmendex %O -o %D %S';
    # dvipdf のビルドコマンド
    ${1}dvipdf = 'dvipdfmx %O -o %D %S';
    # dvipd のビルドコマンド
    ${1}dvips = 'dvips %O -z -f %S | convbkmk -u > %D';
    ${1}ps2pdf = 'ps2pdf.exe %O %S %D';

    # PDF の作成方法を指定するオプション
    ## ${1}pdf_mode = 0; PDF を作成しない。
    ## ${1}pdf_mode = 1; $pdflatex を利用して PDF を作成。
    ## ${1}pdf_mode = 2; $ps2pdf を利用して .ps ファイルから PDF を作成。
    ## ${1}pdf_mode = 3; $dvipdf を利用して .dvi ファイルから PDF を作成。
    ## ${1}pdf_mode = 4; $lualatex を利用して .dvi ファイルから PDF を作成。
    ## ${1}pdf_mode = 5; xdvipdfmx を利用して .xdv ファイルから PDF を作成。
    ${1}pdf_mode = 4;

    # PDF viewer の設定
    ${1}pdf_previewer = "start %S";  # "start %S": .pdf に関連付けられた既存のソフトウェアで表示する。
    ...
    # ${1}pdf_previewer = '{0} -reuse-instance';
'@ -f $sumatraPath ,'$'
...
if (Test-Path "$env:USERPROFILE/.vscode/latex-example") {
